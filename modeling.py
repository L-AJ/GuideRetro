...
class Transformer(nn.Module):
    def __init__(self, config):
        ...
        # ==================== fusion module ====================
        self.fusion_dim = 512  # dim of fusion space
        self.fusion_dropout = 0.1  # dropout rate

        # ------------------ external feature attention ------------------
        self.external_attn = nn.MultiheadAttention(
        ...
        # ------------------ Dropout  ------------------
        self.attn_dropout = nn.Dropout(self.fusion_dropout)
        self.fusion_drop = nn.Dropout(self.fusion_dropout)

        # ------------------ encoder <-> external ------------------
        self.enc2ext = nn.Linear(config.embedding_size, self.fusion_dim)
        self.ext2enc = nn.Linear(self.fusion_dim, config.embedding_size)
        self.norm = nn.LayerNorm(self.fusion_dim)

        # -------------------- external_features FFN --------------------
        self.ext_ffn = nn.Sequential(
            nn.Linear(self.fusion_dim, self.fusion_dim),
            nn.GELU(),
            nn.Dropout(self.fusion_dropout),
            nn.Linear(self.fusion_dim, self.fusion_dim)
        )
        self.ext_norm = nn.LayerNorm(self.fusion_dim)
        ...
        self.enc_norm = nn.LayerNorm(self.fusion_dim)

        # -------------------- gated fusion after fusion --------------------
        self.gate = nn.Linear(self.fusion_dim, self.fusion_dim)

        # -------------------- ext2enc FFN --------------------
        self.ext2enc_ffn = nn.Sequential(
            nn.Linear(config.embedding_size, 4 * config.embedding_size),
            nn.GELU(),
            nn.Dropout(self.fusion_dropout),
            nn.Linear(4 * config.embedding_size, config.embedding_size)
        )
        self.ext2enc_norm = nn.LayerNorm(config.embedding_size)

    def forward(self, products_ids, reactants_ids,
                products_mask, reactants_mask,
                mutual_mask, memory_mask, external_features):
        """
        Args:
            products_ids: [batch, seq]
            reactants_ids: [batch, seq]
            products_mask: [batch, seq]
            reactants_mask: [batch, seq]
            mutual_mask: [batch, seq]
            memory_mask: [batch, ...]
            external_features: [batch, depth, fusion_dim] 或 [batch, fusion_dim]
        
        Returns:
            predict_output: [batch, seq, vocab_size]
        """
        # ====== embedding ======
        ...
        encoder_output = self.lin(encoder_output)  # [batch, seq, embed]

        # =============================================================
        #    global and local fusion
        # =============================================================

        # ---------- external_features FFN + Norm ----------
        # external_features: [batch, depth, fusion_dim]
        ext_feat = self.ext_ffn(external_features)
        ext_feat = self.ext_norm(ext_feat + external_features)  

        # ---------- encoder_output → fusion_dim ----------
        # encoder_output: [batch, depth, seq, embed] -> [batch, depth, seq, fusion_dim]
        enc_proj = self.enc2ext(encoder_output)

        # ---------- enc_proj FFN + Norm ----------
        enc_proj_res = self.enc_ffn(enc_proj)
        enc_proj = self.enc_norm(enc_proj_res + enc_proj)  # 残差连接

        
        # enc_proj: [batch, depth, seq, fusion_dim]
        # ext_feat: [batch, depth, fusion_dim]
        B, D, S, H = enc_proj.shape

        # Query
        # [batch, depth, seq, fusion_dim] -> [batch*depth, seq, fusion_dim]
        enc_proj_flat = enc_proj.view(B * D, S, H)

        # Key/Value
        # [batch, depth, fusion_dim] -> [batch*depth, fusion_dim] 
        # -> [batch*depth, 1, fusion_dim] -> [batch*depth, seq, fusion_dim]
        ext_feat_flat = ext_feat.view(B * D, H).unsqueeze(1).expand(-1, S, -1)

        fused_flat, attn_weights = self.external_attn(
            query=enc_proj_flat,
            key=ext_feat_flat,
            value=ext_feat_flat
        )

        fused_flat = self.attn_dropout(fused_flat)
        fused = fused_flat.view(B, D, S, H)

        # ---------- gated  ----------
        gate_val = torch.sigmoid(self.gate(fused))
        fused = fused * gate_val + enc_proj * (1 - gate_val)

        # ---------- LayerNorm + Dropout ----------
        fused = self.norm(fused)
        fused = self.fusion_drop(fused)

        encoder_output = self.ext2enc(fused)
        encoder_output_res = self.ext2enc_ffn(encoder_output)
        encoder_output = self.ext2enc_norm(encoder_output_res + encoder_output)
