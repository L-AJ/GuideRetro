...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument('--fp_path', type=str, default=None,
                  help='Path to the pretrained fingerprint numpy file (.npy). '
                       'Shape must be [n_entities, hidden_dim].')
...
class ExternalEmbedding:
    ...
    # 修改 ExternalEmbedding 类中的 init 方法
    def init(self, emb_init, fp_path=None):
        if fp_path is not None and os.path.exists(fp_path):
            print(f"[Init] Loading fingerprints from {fp_path}...")
            
            # 1. 加载数据
            fp_data = np.load(fp_path) # Shape: (N, 256) dtype=uint8
            
            # 2. 判断是否是压缩格式 (Packed) 并解压
            # 典型的 Morgan 指纹压缩后是 256 维 (2048 bits)
            if fp_data.dtype == np.uint8 and fp_data.shape[1] == 256:
                print("[Init] Detected packed bits (uint8). Unpacking to 2048-dim vectors...")
                
                # 使用 numpy 进行解压: [N, 256] -> [N, 2048]
                # 这一步会增加 8 倍内存，如果 N 很大，需要注意内存
                fp_unpacked = np.unpackbits(fp_data, axis=1) 
                
                # 转为 Tensor
                fp_tensor = th.from_numpy(fp_unpacked).float()
            else:
                # 假设已经是解压好的或者原本就是低维特征
                fp_tensor = th.from_numpy(fp_data).float()
                
            num_nodes, fp_dim = fp_tensor.shape
            target_dim = self.emb.shape[1]
            
            # --- 关键修改：处理维度不匹配 ---
            if fp_dim != target_dim:
                print(f"[Init] Projecting FP ({fp_dim}) to Embedding ({target_dim})...")
                
                # 定义投影矩阵
                projection_matrix = th.empty(fp_dim, target_dim)
                nn.init.xavier_uniform_(projection_matrix)
                
                # 投影： Emb = FP * W
                # 这里可能消耗大量内存，建议分块处理或者放在 GPU 上
                projected_emb = th.matmul(fp_tensor, projection_matrix)
                
                # 归一化
                projected_emb = th.nn.functional.normalize(projected_emb, p=2, dim=1) * emb_init
                
                self.emb.data.copy_(projected_emb)
            else:
                self.emb.data.copy_(fp_tensor)

            INIT.zeros_(self.state_sum)
            print("[Init] Fingerprint-based initialization done.")
                
        else:
            # 原始逻辑：随机初始化
            if fp_path is not None:
                print(f"[Init] Warning: fp_path {fp_path} not found.")
            print("[Init] Using Random Uniform Initialization.")
...
class KEModel(object):
    ...
    # 修改 KEModel 类中的 reset_parameters 方法
    def reset_parameters(self):
        """Re-initialize the model."""
        # 获取参数中的 fp_path
        fp_path = getattr(self.args, 'fp_path', None)
        
        # 传递给 Entity Embedding 的 init
        self.entity_emb.init(self.emb_init, fp_path=fp_path)
...
class KGDataset:
    ...
    * test stores the triples in the test set.
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0, 1, 2]):
        ...
        tails = []
        rels = []
        # filter_keywords = ['eval', 'valid', 'val', 'test','mit','50k']
        with open(path) as f:
            if skip_first_line:
                _ = f.readline()
            for line in f:
                triple = line.strip().split(self.delimiter)
                # 关系为reactant_to_product
                # if int(triple[format[1]]) == 1 and any(keyword in triple[3] for keyword in filter_keywords):
                #     continue
    ...
    def read_triple(self, path, mode, skip_first_line=False, format=[0, 1, 2]):
        ...
        with open(path) as f:
            ...
            for line in f:
                ...
                # if int(triple[format[1]]) == 1 and any(int(triple[3])==keyword for keyword in filter_keywords):
...
# DGLBACKEND=pytorch python pretrain_kg_embedding_FP.py --model_name TransE_l2 --dataset Embedding  --data_path Data/Train/for_embedding --data_files all_molecules_clean.txt  relations.txt  clean_reactions.txt   --format udd_hrt --batch_size 2048 --neg_sample_size 128 --hidden_dim 512 --gamma 12.0 --lr 0.1 --max_step 500000 --log_interval 1000 --batch_size_eval 16 -adv --regularization_coef 1.00E-07 --gpu 0 --fp_path Data/Train/for_embedding/fingerprints_packed.npy
